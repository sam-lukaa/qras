...
export const useLecturerSignup = () => {
  const { push } = useRouter()

  const setUSerData = useSetRecoilState(userData)

  const postFn = (data: any) => axios.post('lecturer/register', data)
...
export const useLecturerSignin = () => {
  const { push } = useRouter()

  const setUSerData = useSetRecoilState(userData)

  const postFn = (data: any) => axios.post('lecturer/login', data)
